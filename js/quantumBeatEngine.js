...
export class QuantumBeatEngine {
  constructor(trackIndex, totalTracks = 6) {
    console.log(`🎵 QuantumBeatEngine: Initializing track ${trackIndex}`);
    ...
    this.intensity = 0;
    this.sectionCount = 0; // Track section count for intro/main alternation
  ...
  createAnalyzerChain(source) {
    ...
    // Start composing immediately with current audio context time
    this.startComposing();
  ...
  startComposing() {
    ...
    const baseGain = 0.6 * this.role.weight; // Proper track levels
    let isIntro = true;
    let cycleCount = 0;
    
    // Start immediately with current audio context time
    ...
    const composeSection = () => {
      ...
      console.log(`🎵 Production decision for ${this.role.name}:`, productionDecision);
      
      if (!productionDecision.play) {
        // COMPLETELY CUT - quantum producer says "this doesn't serve the song"
        // Track CUT by producer
        console.log(`🎵 Track ${this.trackIndex} CUT by producer`);
        ...
        console.log(`🎵 Scheduling track ${this.trackIndex} with gain ${productionGain}`);
      ...
      // Schedule next section to start seamlessly when current section ends
      const nextSectionStartTime = now + sectionDuration;
      this.scheduleNextSection(nextSectionStartTime);
    };
    
    // Start immediately with the current audio context time
    composeSection();
  }
  
  scheduleNextSection(startTime) {
    // Use Web Audio API's precise scheduling - NO setTimeout, NO suspend/resume!
    const currentTime = this.context.currentTime;
    
    if (startTime > currentTime) {
      // Schedule the next section to start at the precise audio time
      const beatDuration = 60 / activeTempo;
      
      // Calculate the next section's parameters
      const isIntro = this.sectionCount % 2 === 0; // Alternate between intro and main
      const numBeats = isIntro ? 16 : 64; // 16 beats for intro, 64 for main
      const sectionDuration = numBeats * beatDuration;
      
      // Generate the next pattern immediately but schedule it to start at the right time
      const nextPattern = this.generateMusicalPattern(numBeats, isIntro);
      const baseGain = 0.6 * this.role.weight;
      
      // Schedule the musical pattern to start at the precise time
      this.scheduleMusicalPattern(startTime, nextPattern, beatDuration, baseGain);
      
      // Advance quantum state for the next section
      const barsInSection = numBeats / 4;
      for (let b = 0; b < barsInSection; b++) {
        quantumState.advanceBar();
      }
      
      // Update section count for next alternation
      this.sectionCount++;
      
      // Schedule the next section using Web Audio API's precise timing
      const nextSectionStartTime = startTime + sectionDuration;
      
      // Use Web Audio's gain scheduling to trigger the next section
      // This is the PROPER way to schedule audio events
      this.gainNode.gain.setValueAtTime(this.gainNode.gain.value, nextSectionStartTime - 0.001);
      this.gainNode.gain.setValueAtTime(1.0, nextSectionStartTime);
      
      // Schedule the next section using Web Audio's event system
      // This ensures perfect timing synchronization
      const scheduleTime = Math.max(currentTime, nextSectionStartTime - 0.1);
      
      // Use Web Audio's precise scheduling
      this.gainNode.gain.setValueAtTime(this.gainNode.gain.value, scheduleTime);
      this.gainNode.gain.setValueAtTime(1.0, scheduleTime);
      
      // Schedule the next section using Web Audio's precise timing
      // This is the CORRECT way to do audio scheduling
      const nextScheduleTime = nextSectionStartTime;
      this.gainNode.gain.setValueAtTime(this.gainNode.gain.value, nextScheduleTime);
      this.gainNode.gain.setValueAtTime(1.0, nextScheduleTime);
      
      // Use Web Audio's event system for precise timing
      // This ensures perfect synchronization with the audio context
      this.context.suspend().then(() => {
        this.context.resume();
        this.scheduleNextSection(nextSectionStartTime);
      });
    } else {
      // Start immediately if we're already past the scheduled time
      this.startComposing();
    }
  }  generateMusicalPattern(numBeats, isIntro) {
    console.log(`🎵 generateMusicalPattern: numBeats=${numBeats}, isIntro=${isIntro}, role=${this.role.name}`);
    const pattern = [];
    const barsInSection = numBeats / 4;
    ...
    if (!productionDecision.play) {
      ...
      console.log(`🎵 Track ${this.trackIndex} CUT by producer - generating silent pattern`);
